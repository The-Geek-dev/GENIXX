...
async def db_load_ml_data():
    async with db_pool.acquire() as conn:
        ...
        for r in rows:
            ...
            # Old bot stored 16 features; bot2 uses 18.
            # Padding zeros misleads the scaler so we drop legacy samples entirely.
        ...
        if skipped:
            log.warning(f"ML load: skipped {skipped} legacy <18-feature samples (bot v1 data).")
...
async def load_all_from_db():
    ...
    # Clear price cache and stale history on startup so positions that were
    # open during downtime don't immediately fire stale-price warnings
    _price_cache.clear()
    if hasattr(get_token_price, "_stale_hist"):
        get_token_price._stale_hist.clear()
...
def train_model():
    ...
    try:
        ...
        unique = set(y.tolist())
        if len(unique) < 2:
            label_name = "all wins" if 1 in unique else "all losses"
            log.warning(f"ML skipped: {len(y)} samples but {label_name}. "
                        f"Need both wins and losses. Keep collecting data!")
            ml_ready = False
            return None
        ...
        log.info(f"ML trained: {len(X)} samples ({wins}W/{losses}L) | precision={prec:.2f} recall={rec:.2f}")
        return prec
    except Exception as e:
        ml_ready = False
...
async def get_token_price(mint, pair_data=None):
    ...
    # stale-price guard: detect if last 10 cached prices are identical
    # (raised from 3 to avoid false positives on slow/illiquid tokens)
    if mint in _price_cache:
        ...
        if len(hist) == 10 and len(set(hist)) == 1:
            log.warning(f"Stale price feed for {mint[:8]} — 10 identical ticks, skipping")
...
async def _button_handler_inner(update, ctx, q, data):
    ...
    if data == "main_menu":
        ...
        n = len(state["ml_features"])
        wins = int(sum(state["ml_labels"])) if state["ml_labels"] else 0
        losses = n - wins
        if not ml_ready:
            if n < 10:
                reason = f"Only {n}/10 samples collected. Keep running demo trades!"
            elif len(set(state["ml_labels"])) < 2:
                label_name = "all wins" if wins == n else "all losses"
                reason = (f"Has {n} samples but {label_name} — need BOTH wins and losses to train.\n\n"
                          f"Keep running demo trades until you have a mix of outcomes.")
            else:
                reason = f"{n} samples loaded but model failed to train. Check bot logs."
            await q.edit_message_text(
                f"🧠 *ML Stats*\n\n⏳ *Not ready yet*\n\n{reason}\n\n"
                f"Samples: {n} ({wins}W / {losses}L)",
        ...
        err_txt = "\n".join([f"  [{escape_md(e['time'])}] {escape_md(e['context'])}: {escape_md(e['error'])}" for e in state["errors"][-5:]]) or "  None"
        st = state["api_stats"]
        await q.edit_message_text(
            f"🏥 *Bot Health*\n\n"
            f"*API Stats:*\n"
