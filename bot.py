...
# The Raydium WS sniper subscribes to ALL logs mentioning the Raydium AMM
# program (one of the highest-throughput programs on Solana), then filters
# client-side for pool-init events. Helius bills per delivered message, so
# this burns credits fast regardless of how few events actually matter.
# Off by default — turn on explicitly once you're ready to pay for it or
# have room in your credit budget. Discovery still works without this via
# the free DexScreener-based _discover polling.
ENABLE_RAYDIUM_WS_SNIPER = os.getenv("ENABLE_RAYDIUM_WS_SNIPER", "false").lower() in ("1", "true", "yes")
...
async def init_db():
    global db_pool
    # Only force SSL when explicitly requested (e.g. connecting to a public/
    # external Postgres host). Railway's internal/private DATABASE_URL does
    # not speak SSL, and forcing it causes the server to reset the connection
    # during the SSL handshake (ConnectionResetError: Connection reset by peer).
    ...
    if use_ssl:
        ...
        ssl_arg = ctx
    db_pool = await asyncpg.create_pool(DATABASE_URL, min_size=1, max_size=5, ssl=ssl_arg,
...
async def post_init(app):
    ...
    if HELIUS_RPC and ENABLE_RAYDIUM_WS_SNIPER:
        asyncio.create_task(raydium_ws_sniper(app))
        log.info("Raydium WS sniper started")
    else:
        log.info("Raydium WS sniper disabled (set ENABLE_RAYDIUM_WS_SNIPER=true to enable — note: burns Helius credits fast)")
